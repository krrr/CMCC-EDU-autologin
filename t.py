...
try:
    ...
    #elif config['GUI']['Type'] == 'KDE':
    #    def myprint(info):
    #        timeout = int(config['GUI']['Balloontimeout'])
    #        os.popen("kdialog --passivepopup 'Cxx-autologin\n\n%s' %s" % (info, timeout))
    else:
        myprint = print
except:
...
try:
    temp = str(opener.open('http://8.8.8.8', timeout=6).read())
except:
    try:
...
iURL = re.search(r'="Wp_frame" id="Wp_frame" src="(.+?)" ', temp).group(1)
...
def login_noc():
    ...
    try:
        lpage = opener.open(lURL, data.encode('utf-8'), timeout=6).read()
    except:
        myprint('Error: Timeout,failed talking with their server')
        sys.exit()
...
def checker(lpage, is_c):
    if 'user_status.php?' in lpage:
        # skip saving cookie when logged in by cookie
        ...
        if timereminder_on:  # Time Reminder
            ...
            try:
                ...
                myprint('Login succeed(Failed getting time usage)')
        else:
